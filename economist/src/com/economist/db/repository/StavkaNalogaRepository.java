...
public interface StavkaNalogaRepository extends JpaRepository<StavkaNaloga, Integer> {
	...
	@Query("SELECT sn FROM StavkaNaloga sn WHERE sn.datum between ? AND ? AND sn.nalog.preduzece = ? AND sn.komitent = ? order by sn.datum asc")
	List<StavkaNaloga> kif(Date datumOd, Date datumDo, Preduzece preduzece, Komitent komitent);
	
	@Query("SELECT sn FROM StavkaNaloga sn WHERE sn.datum between ? AND ? AND sn.nalog.preduzece = ? order by sn.datum asc")
